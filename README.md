...
| 20       | —            | Cosine      | 128            | 否             | 16,32,32              | 未獲得                  | 0.5392                  | 穩定                                     | 批量大小變化是解決 Roberta 不穩定性的關鍵因素。                         |
...
* `No_17(score_0_85167).py`：第17版的完整訓練腳本（最高分模型）
...
* `No_17(score_0_85167).ipynb`：第17版的完整 Jupyter Notebook（最高分模型）
...
2. 執行模型：
   ...
   * 使用 `ipynb/`：
     ...
     ```bash
     cd ipynb/
     jupyter notebook No_17(score_0_85167).ipynb
     ```
